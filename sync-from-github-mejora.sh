...
# --- MEJORA 1: Manejo de interrupciones ---
cleanup() {
...
# --- MEJORA 2: Contadores para resumen final ---
SUCCESS_COUNT=0
SKIP_COUNT=0
ERROR_COUNT=0
...
for current_repo in $REPO_LIST; do
    ...
    if ! git pull $GIT_PULL_ARGS > /dev/null; then
        ...
        ERROR_COUNT=$((ERROR_COUNT + 1))
        # En lugar de exit, volvemos al directorio base para seguir con el siguiente repo
        cd - > /dev/null
        SUCCESS_COUNT=$((SUCCESS_COUNT + 1))
    fi
    ...
    echo "Successfully updated/cloned: $SUCCESS_COUNT"
    echo "Skipped (filters): $SKIP_COUNT"
    echo "Failed: $ERROR_COUNT"
    echo "------------------------------------------------"
fi# --- MEJORA 3: Resumen final de ejecución ---
if ! $QUIET; then
    echo "------------------------------------------------"
    elif ! $QUIET; then
